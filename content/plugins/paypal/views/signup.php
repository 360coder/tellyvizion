        ...
        <div class="panel-heading">
          ...
          <div class="row">
              ...
              <div class="cc-icons col-lg-5 col-md-4">
                  <img src="/content/themes/default/assets/img/credit-cards.png" alt="All Credit Cards Supported" />
              </div>
              <?php else: ?>
        ...
        <div class="panel-body">
          ...
          <fieldset>
            ...
            <div class="form-group row">
                ...
                <?php include('partials/paypal-forms.php'); ?>
            </div>
...
<script type="text/javascript" src="/content/themes/default/assets/js/jquery.payment.js"></script>
<script type="text/javascript">
  ...
  <?php if( ! empty($plugin_data->paypal_merchant_id)) : ?>
    jQuery(function($) {
        var paypal_buttons = $('button.paypal-button');
        paypal_buttons.click(function(e) {
            e.preventDefault();

            $.ajax({
                url: '/ajax_signup',
                method: 'post',
                data: 'username=' + $('#username').val() + '&email=' + $('#email').val() + '&password=' + $('#password').val() + '&password_confirmation=' + $('#password_confirmation').val() + '&_token=' + $('input[name="_token"]').val(),
                context: this,
                beforeSend: function() {
                    paypal_buttons.attr('disabled', 'disabled');
                },
                success: function(r) {
                    var m, x, note = '', fields = ['note', 'username', 'email', 'password', 'password_confirmation'];

                    try {
                        m = $.parseJSON(r);
                    }
                    catch (e) {
                        noty({text: 'Sorry, there was an error creating your account.', layout: 'top', type: 'error', timeout:1300 });
                        return false;
                    }
                ...
                complete: function() {
                    paypal_buttons.removeAttr('disabled');
                }
            });
...
<style type="text/css"><?php include('content/plugins/paypal/assets/css/style.css'); ?></style>
