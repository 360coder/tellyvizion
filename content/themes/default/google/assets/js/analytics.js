function Analytics(){
	...
	this.init= function(){
        ...
        $('.listCore a').click(function(){
            _id = $(this).attr("href");
            $('html,body').animate({ scrollTop: $(_id).offset().top - 80},'medium');
            return false;
        });
    ...
    this.chart = function(){
        _timeout = 0;
        setTimeout(function(){ self.ajax_chart('viewchart'); },_timeout);

        _timeout += 2000;
        setTimeout(function(){ self.ajax_chart('watchchart'); },_timeout);

        _timeout += 2000;
        setTimeout(function(){ self.ajax_chart('engagementchart'); },_timeout);

        _timeout += 2000;
        setTimeout(function(){ self.ajax_chart('revenuechart'); },_timeout);
        ...
        _timeout += 2000;
        setTimeout(function(){ self.ajax_chart('annotationschart'); },_timeout);

        _timeout += 8000;
        setTimeout(function(){ self.ajax_chart('devicechart'); },_timeout);
    ...
    this.ajax_chart = function(element){
        ...
        $.post(PATH + 'youtube_analytics/google_ajax_' + element, _data, function(data){
            $('.' + element).html(data);
