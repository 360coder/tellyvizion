function Page(){
	...
	this.init= function(){
        ...
        $(document).on('click', '.btnActionModule', function(){
            ...
            _data     = _data + '&' + $.param({_token:token, action: _type});
            ...
            if(_valid > 0){
                if(_type == "delete"){
                    self.showConfirmMessage(_confirm, function(){
                        $.post(_action, _data, function(result){
                            ...
                            self.showSuccessAutoClose("deleted", "success", 2000);
                        },'json');
                    });
                }else{
                    $.post(_action, _data, function(result){
                ...
                self.showSuccessAutoClose("selectoneitem", "info", 2000);
            }
        ...
        $(document).on('click', '.btnActionModuleItem', function(){
            ...
            if(_type == "delete"){
                _data    = $.param({_token:token, action: _type, id: _id});
                self.showConfirmMessage(_confirm, function(){
                    $.post(_action, _data, function(result){
                        setTimeout(function(){
                            window.location.reload();
    ...
    this.Search = function(){
        ...
        $(document).on("click", ".btnSearchGroups",  function(){
            ...
            if(!_that.hasClass('disable')){
                ...
                $(".page-loader-action").fadeIn();
                $.post(_action, {_token: token, limit: _limit, account: _account, keyword: _keyword, type: _type, channel: _channel}, function(result){
