...
//include(app_path() . '/lib/Mixpanel.php');
	...
	*/
	...
	public function login(){
	    ...
	    if ( Auth::attempt($email_login) || Auth::attempt($username_login) ){
	        ...
	        return Redirect::to('/' . $redirect)->with(array('note' => 'Invalid login, please try again.', 'note_type' => 'error'));
	    }
	...
	public function signup(){
		...
		if ($validation->fails()){
			return Redirect::to('/')->with(array('note' => 'Sorry, there was an error creating your account.', 'note_type' => 'error', 'messages'))->withErrors($validation)->withInput(\Request::only('username', 'email'));
		}
	    ...
	    try{
	    	...
	    	Auth::logout();
	    	$user->delete();
	    	return Redirect::to('/')->with(array('note' => 'Sorry, there was an error with your card: ' . $e->getMessage(), 'note_type' => 'error'))->withInput(\Request::only('username', 'email'));
	    }
	...
	public function verify($activation_code){
		if( ! $activation_code)
        {
            ...
            return Redirect::to('/')->with(array('note' => 'Confirmation Code is empty', 'note_type' => 'error'));
        }
        ...
        {
            //throw new InvalidConfirmationCodeException;
             return Redirect::to('/')->with(array('note' => 'Invalid Confirmation Code', 'note_type' => 'error'));
        }
