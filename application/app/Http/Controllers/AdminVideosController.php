...
class AdminVideosController extends \BaseController {
    ...
    public function index()
    {
        ...
        if(!empty($search_value)):
            $videos = Video::where('videos.title', 'LIKE', '%'.$search_value.'%')->join('users', 'videos.user_id', '=', 'users.id')->orderBy('videos.created_at', 'desc')->paginate(9);
        else:
            $videos = Video::orderBy('videos.created_at', 'DESC')->join('users', 'users.id', '=', 'videos.user_id')->select('users.name', 'users.username', 'videos.id', 'videos.title', 'videos.user_id', 'videos.active', 'videos.image', 'videos.featured','videos.sliders')->paginate(15);
        endif;
    ...
    {
        $video = Video::find($id);
        $active = ($video->active == '1') ? '0' : '1' ;
        $message = ($video->active == '1') ? 'Inactive' : 'Active' ;
        $data['active'] = $active;
        $video->update($data);
		$activities = array(
		'user_id' => $data['active'],
        ...
        return Redirect::to('admin/videos')->with(array('note' => 'Successfully marked the Video as '.$message, 'note_type' => 'success') );
    }
