			...
			<div class="panel panel-primary" data-collapsed="0"> <div class="panel-heading"> 
				...
				<div class="panel-body" style="display: block;"> 
					...
					<div class="new-video-file" @if(!empty($video->type) && $video->type == 'file'){{ 'style="display:block"' }}@endif>
						<input type="file" class="form-control" name="video" id="video" value="@if(!empty($video->video)){{ $video->video }}@endif" />
					</div>
				...
				<div class="col-sm-4"> 
					<div class="panel panel-primary" data-collapsed="0"> 
						...
						<div class="panel-body"> 
							...
							<div class="clear"></div>
							<div>
								<label for="sliders" style="float:left; display:block; margin-right:10px;">Use Video As Slider:</label>
								<input type="checkbox" @if(!empty($video->sliders) && $video->sliders == 1){{ 'checked="checked"' }}@endif name="sliders" value="1" id="sliders" />
							</div>
...
<!-- This is where now -->
	...
	<script type="text/javascript" src="{{ URL::to('/application/assets/js/tagsinput/jquery.tagsinput.min.js') }}"></script>
